# -*- coding: utf-8 -*-
"""
Created on Fri Sep  9 10:51:26 2022

@author: UX325
"""

# Algoritmo de Optimización por Enjambre de Partículas aplicado al problema del puerto XYZ.
...
for ejecucion in range(20): # Bucle para realizar las 20 ejecuciones para la medición de la eficiencia.
    
    # 1. INICIALIZACIÓN
    # Configuración de parámetros.
    N = 5 # Número de partículas (camiones)
    M = 15 # Número de tareas.
    d = 2 # Dimensión del espacio de búsqueda 
    P = 5 # Número de puertos incluyendo el depósito.
    lb = 400 # Límite inferior del espacio de búsqueda
    ub = 1200 # Límite superior del espacio de búsqueda
    it = 0 # Iteración inicial
    it_max = 70 # Número máximo de iteraciones (criterio de paro: k = it_max)
    c1 = 2 # Constante cognitiva
    c2 = 5 # Constante social
    Evolucion = np.zeros(it_max) # Vector con los mejores fitness globales
    ...
    class Particula():
        ...
        def __init__(self):
            ...
            Atributos de instancia:
                posicion (numpy.ndarray) : posición de la partícula (camión) de dimensión M + 2*N.
                
                velocidad (numpy.ndarray) : velocidad de la partícula (camión) de dimensión M + 2*N.
                
                fit (numpy.ndarray) : valor de fitness de la partícula (camión).
                
                mejor_fitness_local (float) : valor del mejor fitness de la partícula (camión).
                
                mejor_posicion_local (numpy.ndarray) : posición de la partícula (camión) cuando alcanza el mejor_fitness_local.
                
                cap (float): capacidad de la partícula (camión).
    
            """
            self.posicion = np.repeat(None, M + 2*N)
            self.velocidad = np.zeros(M + 2*N)
    ...
    # INICIALIZACIÓN
    for k in range(N):
        Particulas[k].posicion = np.random.uniform(lb, ub, size = M + 2*N)
        posiciones[k] = Particulas[k].posicion
        Particulas[k].mejor_posicion_local = Particulas[k].posicion
        Particulas[k].cap = np.random.uniform(700, 1300)
        capacidades[k] = Particulas[k].cap
    ...
    while it < it_max: # Criterio de paro
        ...
        for i in range(N):
            ...
            Particulas[i].fit = funObj(dist_vac,dist_tot,fuel,impacto_med,tiempo_espera, penalizaciones)
            
            if it == 1:
                Particulas[i].mejor_fitness_local = Particulas[i].fit
                Particulas[i].mejor_posicion_local = Particulas[i].posicion
                
                if i == 0: # Inicializamos el mejor fitness global como el fitness de la primera partícula en la primera iteración.
            ...
            # Actualizamos la velocidad de la partícula i.
            ...
            posiciones[i] = Particulas[i].posicion
            # Verificar que las partículas no se salgan de los límites lb y ub:
            for i in range(N):
                if any(Particulas[i].posicion < lb):
                    posiciones_erroneas = np.where(Particulas[i].posicion < lb)
                    Particulas[i].posicion[posiciones_erroneas] = lb
                elif any(Particulas[i].posicion > ub):
                    posiciones_erroneas = np.where(Particulas[i].posicion > ub)
                    Particulas[i].posicion[posiciones_erroneas] = ub
    ...
    nombreFichero = "PSO_PuertoXYZ_num_particulas_=" + str(N) + ".txt"
